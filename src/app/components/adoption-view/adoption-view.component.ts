...
export class AdoptionViewComponent {
  ...
  @Output() snapshotsChange = new EventEmitter<AdoptionSnapshot[]>();
  ...
  get isDirty(): boolean {
    if (this.adoptionMode !== 'adoption' || this.candidateEmployees.length === 0) {
      return false;
    }
    if (!this.activeSnapshotId) {
      return true;
    }
    const activeSn = this.snapshots.find(s => s.id === this.activeSnapshotId);
    if (!activeSn) return true;

    return JSON.stringify(this.candidateEmployees) !== JSON.stringify(activeSn.candidates);
  }

  onModeChange(mode: AdoptionMode): void {
    if (mode === 'standard' && this.candidateEmployees.length > 0) {
      this.draftCandidates = JSON.parse(JSON.stringify(this.candidateEmployees));
    } else if (mode === 'adoption' && this.candidateEmployees.length === 0 && this.draftCandidates.length > 0) {
      this.candidatesChange.emit(JSON.parse(JSON.stringify(this.draftCandidates)));
    }
  ...
  onApplyPersona(persona: AdoptionPersona): void {
    this.adoptionControlRef?.applyPersonaToCandidates(persona);
  }
  ...
  saveCurrentSnapshot(): void {
    if (!this.selectedScenario || !this.selectedBase100Scenario) return;

    const salesDiff = (this.selectedScenario.totalSales || 0) - (this.selectedBase100Scenario.totalSales || 0);
    const profitDiff = (this.selectedScenario.totalProfit || 0) - (this.selectedBase100Scenario.totalProfit || 0);
    
    const count = this.candidateEmployees.length;
    const name = this.newSnapshotName.trim() || `追加試算 (${count}名体制)`;
    ...
    const updated = [snapshot, ...this.snapshots];
  ...
  applySnapshot(snapshot: AdoptionSnapshot): void {
    this.activeSnapshotId = snapshot.id;
    this.onModeChange('adoption');
    this.onCandidatesChange(JSON.parse(JSON.stringify(snapshot.candidates)));
  }
}